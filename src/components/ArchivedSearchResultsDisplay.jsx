// src/components/ArchivedSearchResultsDisplay.jsx
import React, { useState } from 'react';
import { Link } from 'react-router-dom';
// Re-use RawDataModal from SearchResults if it's exported or define a similar one here
// For simplicity, assuming RawDataModal is part of SearchResults.jsx and not easily reusable
// We'll define a simple one here or you can refactor RawDataModal to be a shared component.

const SimpleRawDataModal = ({ data, onClose, title = "Raw Data" }) => {
  if (!data) return null;
  // Basic modal styling, you might want to reuse/enhance this
  const overlayStyle = {
          ...
          <ul className="places-list">
            {fetchedMatches.map((item, index) => (
              <li key={item.id || item.place_id || `archive-match-${index}`} className="place-item">
                ...
                {/* Display the match_score from the item */}
                <p>
